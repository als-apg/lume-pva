...
| Add a CI leg that runs the suite without the `torch` extra | Every other leg installs everything, so a module-scope import of an optional dependency passes them all and then takes collection — the whole run, not one file — down for any consumer who installed less. The leg also asserts every shipped test module contributed a passing test, so a guard widened until it skips whole modules cannot stay green either. |
| Add a CI leg that installs the built sdist and runs the suite out of it | Every other leg installs from the checkout, where every file is present whether or not the packaging ships it, so a file left out of the sdist passes the whole suite and fails on the first consumer to install the release — after the version number is spent. |
| Guard the test suite's optional imports so a partial install skips rather than failing collection | The suite imported `epics`, `torch` and `p4p` at module scope, so anything short of the full dev-plus-torch set died during collection and ran nothing at all — including the tests that needed none of them. Guarding turns "not installed" into a reported skip of the one module that could not run. |
| Report an absent optional dependency as a `ModuleNotFoundError` carrying `name` | Absent and broken were distinguished in the message but not in the exception, so a caller reraising one — the suite's own import guards do — saw every failure as the broken case and could not decide whether to skip. |
| Add tests for what an install guarantees | The pure-Python core is a property of what the modules import, not only of what the wheel declares: a convenience re-export in `__init__` would pull a transport into the core install while the metadata still called it pure. Asserted by importing in a fresh interpreter with the optional dependencies made unimportable, which also covers the two answers given for an unusable transport. |
